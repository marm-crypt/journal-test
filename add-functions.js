...
// Find and remove the old calcMoodReflection function
const reflectionStartMarker = '/* ---------------------------\n   New: meaningful mood reflection';
const reflectionEndMarker = '\n/* ---------------------------\n   Dashboard\n   ----------------------------';

const startIdx = content.indexOf(reflectionStartMarker);
const endIdx = content.indexOf(reflectionEndMarker, startIdx);
...
`;
  ...
  const newContent = content.slice(0, startIdx) + newFunctions + content.slice(endIdx);
